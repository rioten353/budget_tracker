...
def add_todo():
    ...
    created_at = datetime.now().strftime(
        "%d/%m %H:%M")  # Get current date and time
    todo = Todo(title.upper(), created_at)  # Create a new Todo object
...
def print_all_todos():
    ...
    for i, todo in enumerate(todos):
        print(
            f"| {i + 1:2} | {todo.title:35} | {todo.created_at:12} | {'✅' if todo.is_completed else '❌':^11}|"
        )
...
def mark_as_complete():
    ...
    try:
        todo_id = int(
            input("Enter the ID of the todo: ")) - 1  # Prompt user for todo ID
